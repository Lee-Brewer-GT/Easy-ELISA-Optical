...
public class FileSelect extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       // ImageButton lineBtn = (ImageButton) findViewById(R.id.lineBtn);
        ...
        //replace this with AA chart View
        /*
        ...
        shareBtn.setOnClickListener((v) -> {
            Log.i("debug","Share button click listener started");
            File finalDatafile = new File(docPath, title[0]);
            Log.i("debug","final data file created");

            if(finalDatafile.exists()) {
                Intent intentShareFile = new Intent(Intent.ACTION_SEND);
                Log.i("debug","Share file intent created");
                intentShareFile.setType(URLConnection.guessContentTypeFromName(finalDatafile.getName()));
                Log.i("debug","intent share file set type");
                Uri uri = FileProvider.getUriForFile(getApplicationContext(), "com.example.provider.opticalelisatest", finalDatafile); //was com.micronanobio.provider.elisapp
                Log.i("debug","file uri");
                intentShareFile.putExtra(Intent.EXTRA_STREAM, uri);
                intentShareFile.setFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                Log.i("debug","uri permission");
